...
@router.post(path="/", response_model=MeetingRoomDB, response_model_exclude_none=True)
...
@router.get(
    path="/", response_model=list[MeetingRoomDB], response_model_exclude_none=True
)
...
async def partially_update_meeting_room(
    ...
    meeting_room = await check_meeting_room_extsts(meeting_room_id, session)
...
@router.get(path="/{meeting_room_id}/reservations", response_model=list[ReservationDB])
async def get_reservations_for_room(
    meeting_room_id: int, session: AsyncSession = Depends(get_async_session)
):
    await check_meeting_room_exists(meeting_room_id=meeting_room_id, session=session)
